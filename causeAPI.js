...
// console.log(config);
console.log('Load cause API ...');

var api = {};

var causeFieldList = ['id', 'user_id', 'title', 'story', 'visibility', 'cause_type', 'cause_related_1','cause_related_2', 'cause_related_3', 'photo', 'beneficiary_flag', 'beneficiary_first_name', 'beneficiary_last_name', 'beneficiary_email', 'updated_on', 'created_on'];

var groupTableName = "karely_user_group";
var userGroupLinkTableName = "karely_user_group_link";
var causeTableName = "karely_cause";
var sessionTableName = "karely_user_session";

api.getCauseList = async function() {

	var sql = knex.select(causeFieldList);
	sql = sql.from(causeTableName);
...
api.getCause = async function(id) {
...
api.addCause = async function(data)
{
 	var result = await knex(causeTableName).insert(data).returning('*'); 
	if (result.length == 0) throw new Error("Insert fail");
	else if (result.length == 1) return result[0];
	else return result;
};

api.updateCauseWithID = async function(data, id)
{
    
 	var result = await knex(causeTableName).update(data).where({'id': id }).returning('id'); 
	if (result.length == 0) throw new Error("Update fail");
	return result[0];
};

api.deleteCauseWithID = async function(id)
{
 	var result = await knex(causeTableName).where({ 'id': id }).delete().returning('id'); 
